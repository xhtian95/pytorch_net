...
class charlevel_BiLSTM_GAT(nn.Module):
    def __init__(self, nfeat, nhid, nclass, in_drop, coef_drop, alpha, nheads, batch,
                 num_lstm, lstm_hidsize, lstm_numlayers):
        super(charlevel_BiLSTM_GAT, self).__init__()
        self.nfeat = nfeat
        self.nhid = nhid  # GAT的隐层单元数
        self.nclass = nclass
        self.in_drop = in_drop
        self.coef_drop = coef_drop
        self.alpha = alpha
        self.nheads = nheads
        self.num_lstm_list = num_lstm  # 整个网路设计的LSTM网络个数
        self.lstm_hidsize = lstm_hidsize  # 网络中的隐层单元数
    ...
    def initHidden(self):
        h0 = torch.rand(self.num_lstm_list[0] * 2, self.batch, self.lstm_hidsize)
        c0 = torch.rand(self.num_lstm_list[0] * 2, self.batch, self.lstm_hidsize)
        return h0, c0
...
if __name__=="__main__":
    ...
    print(a.size())
    print(a)
    net = charlevel_BiLSTM_GAT(nfeat=n_features, nhid=hidden_units, nclass=1, in_drop=0, coef_drop=0, alpha=0.2,
                               nheads=8, batch=3, num_lstm=num_lstm, lstm_hidsize=8, lstm_numlayers=2)
